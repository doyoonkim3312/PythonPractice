...
def main():
    ...
    monthCounter, dayCounter, monthlySum, monthlyAverage = 1, 0, 0, 0.0
    ...
    for steps in dailySteps.split("\n"):
        ...
        try:
            monthlySum = monthlySum + float(int(steps))
        except ValueError:
            break

        if monthCounter < 8:
            if dayCounter == 31 and (monthCounter % 2) == 1:
                monthlySteps.append(float(monthlySum / 31))
                dayCounter = 0
                monthlySum = 0.0
                monthCounter = monthCounter + 1
            elif dayCounter == 30 and (monthCounter % 2) == 0:
                monthlySteps.append(float(monthlySum / 30))
                dayCounter = 0
                monthlySum = 0.0
                monthCounter = monthCounter + 1
            elif dayCounter == 28 and monthCounter == 2:
                monthlySteps.append(float(monthlySum / 28))
                dayCounter = 0
                monthlySum = 0.0
                monthCounter = monthCounter + 1
        else:
